 return FHIR.oauth2.ready(getA1Cdata, showError);
    var smart1 = null;
    function getA1Cdata(smart) {
      smart1 = smart;
      ////////////////
      //render current patient
      smart1.patient.read().then(
        function (pt) {
          // var patientInfo = [];
          // var patientInfo = [];
       ...
       let container = document.getElementById('containerId'); let tblRow=null; let tblCell=null;tblRow = document.createElement('div');tblRow.classList.add('row'); tblCell = document.createElement('div');tblCell.className='col-sm-6';tblCell.setAttribute('style', 'padding: 40px;');tblRow.appendChild(tblCell);displayBarChart(tblCell,'Observation','LAB (Createnine)');; tblCell = document.createElement('div');tblCell.className='col-sm-6';tblCell.setAttribute('style', 'padding: 40px;');tblRow.appendChild(tblCell);displayLine(tblCell,'Observation','VITAL SIGN (BMI)');;container.appendChild(tblRow);tblRow = document.createElement('div');tblRow.classList.add('row'); tblCell = document.createElement('div');tblCell.className='col-sm-6';tblCell.setAttribute('style', 'padding: 40px;');tblRow.appendChild(tblCell);displayTreeView(tblCell,'MedicationRequest','Text|Status|Intent');; tblCell = document.createElement('div');tblCell.className='col-sm-6';tblCell.setAttribute('style', 'padding: 40px;');tblRow.appendChild(tblCell);displayTable(tblCell,'Procedure','Text|Status|PerformedPeriodEnd|PerformedPeriodstart');;container.appendChild(tblRow);
    }
    ...
    function displayTreeView(container, type, params) {
      var paramList = params.split("|");
      smart1.patient.api
        .search({
